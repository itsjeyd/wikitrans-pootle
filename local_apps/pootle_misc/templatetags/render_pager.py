...
def render_pager(pager):
    ...
    if pager.has_previous():
        result += '<li><a href="?page=1" class="nth-link">%s</a></li>' % \
                  _('First')
        result += '<li><a href="?page=%d" class="prevnext-link">' \
                  '%s</a></li>' % (pager.previous_page_number(),
                                   _('Previous'))
    ...
    for i in range(start, end+1):
        if i == pager.number:
            ...
            result += '<li><a href="?page=%d" class="number-link">' \
                      '%d</a></li>' % (i, i)
    ...
    if pager.has_next():
        result += '<li><a href="?page=%d" class="prevnext-link">' \
                  '%s</a></li>' % (pager.next_page_number(), _('Next'))
