...
from pootle_app.models.permissions import get_matching_permissions, \
     check_permission, check_profile_permission
    ...
    if not profile.alt_src_langs.count():
        ...
        for accept_lang, unused in parse_accept_lang_header(accept):
            ...
            normalized = to_locale(
                data.normalize_code(data.simplify_to_common(accept_lang)))
            code = to_locale(accept_lang)
            if normalized in (
                'en', 'en_US', source_language.code, language.code) or \
                code in (
                'en', 'en_US', source_language.code, language.code):
                continue
...
def get_search_step_query(translation_project, form, units_queryset):
    ...
    if translation_project.indexer is None:
        logging.debug(
            u"No indexer for %s, using database search",
            translation_project)
        return get_non_indexed_search_step_query(form, units_queryset)

    logging.debug(
        u"Found %s indexer for %s, using indexed search",
    ...
    paths = units_queryset.order_by().values_list(
        'store__pootle_path', flat=True).distinct()
    path_querylist = [('pofilename', pootle_path) for pootle_path in
                      paths.iterator()]
    cache_key = "search:%s" % str(
        hash((repr(path_querylist), translation_project.get_mtime(),
              repr(words), repr(fields))))

    dbids = cache.get(cache_key)
    if dbids is None:
        searchparts = []
        # Split the search expression into single words. Otherwise
        # xapian and lucene would interprete the whole string as an
        # "OR" combination of words instead of the desired "AND".
        for word in words:
            # Generate a list for the query based on the selected fields
            word_querylist = [(field, word) for field in fields]
            textquery = translation_project.indexer.make_query(
                word_querylist, False)
            searchparts.append(textquery)

        pathquery = translation_project.indexer.make_query(
            path_querylist, False)
        searchparts.append(pathquery)
        limitedquery = translation_project.indexer.make_query(
            searchparts, True)

        result = translation_project.indexer.search(
            limitedquery, ['dbid'])
        ...
        if unitstates:
            ...
            for unitstate in unitstates:
                if unitstate == 'untranslated':
                    state_queryset = state_queryset | units_queryset. \
                                     filter(state=UNTRANSLATED)
...
def get_current_units(request, step_queryset, units_queryset):
    """
    returns current active unit, and in case of POST previously active
    unit
    """
    edit_unit = None
    prev_unit = None
    ...
    if 'unit' in request.GET:
        ...
        pager = paginate(
            request, units_queryset, items=unit_rows, orphans=0)
        ...
        if back:
            queryset = (
                step_queryset.filter(
                    store__pootle_path=pootle_path,
                    index__lte=prev_index) | \
                step_queryset.filter(
                    store__pootle_path__lt=pootle_path)).order_by(
                '-store__pootle_path', '-index')
        else:
            queryset = (
                step_queryset.filter(
                    store__pootle_path=pootle_path,
                    index__gte=prev_index) | \
                step_queryset.filter(
                    store__pootle_path__gt=pootle_path)).order_by(
                'store__pootle_path', 'index')

        # FIXME: instead of using an arbitrary limit it would be best
        # to page through mother query
        for unit in queryset[:64].iterator():
            ...
            if unit.id == prev_id:
                prev_unit = unit
            elif unit.index > prev_index or back and \
                     unit.index < prev_index:
                logging.debug(
                    u"submitting to a unit no longer part of " \
                    "step query, %s:%d", (pootle_path, prev_id))
                # prev_unit no longer part of the query, load it directly
                edit_unit = unit
                prev_unit = Unit.objects.get(
                    store__pootle_path=pootle_path, id=prev_id)
                break
...
def translate_page(request, units_queryset, store=None):
    ...
    if 'search' in request.GET and 'sfields' in request.GET:
        ...
        if search_form.is_valid():
            step_queryset = get_search_step_query(
                request.translation_project, search_form, units_queryset)
    else:
        search_form = SearchForm()
    ...
    prev_unit, edit_unit, pager = get_current_units(
        request, step_queryset, units_queryset)

    # time to process POST submission
    form = None
    if prev_unit is not None and \
           ('submit' in request.POST or
            'suggest' in request.POST):
        # check permissions
        if 'submit' in request.POST and not \
               cantranslate or 'suggest' in \
               request.POST and not cansuggest:
    ...
    if store is None:
        ...
        preceding = (
            pager_query.filter(
                store=store, index__lt=edit_unit.index) | \
            pager_query.filter(
                store__pootle_path__lt=store.pootle_path)).count()
        store_preceding = store.units.filter(
            index__lt=edit_unit.index).count()
    else:
        pager_query = store.units
        preceding = pager_query.filter(
            index__lt=edit_unit.index).count()
    ...
    if pager is None:
        ...
        pager = paginate(
            request, pager_query, items=unit_rows, page=page, orphans=0)
    ...
    if store_preceding > context_rows:
        ...
        if page_length < unit_rows:
            ...
            units = paginate(
                request, units_query, items=unit_rows, page=page,
    ...
    for check in request.GET.get('matchnames','').split(','):
        ...
        link = '<a href="http://translate.sourceforge.net/wiki/' \
               'toolkit/pofilter_tests#%s" target="_blank">%s</a>' % \
               (safe_check, safe_check)
        checks.append(_('checking %s', link))

    # precalculate alternative source languages
    alt_src_langs = get_alt_src_langs(
        request, profile, translation_project)
