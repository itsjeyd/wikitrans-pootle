...
# Intentionally silence DeprecationWarnings about os.popen3 in Python 2.6.
# It's still sensible for us to use it, since subprocess didn't exist in 2.3.
warnings.filterwarnings(
    'ignore', category=DeprecationWarning, message=r'os\.popen3')
...
def make_messages(
    ...
    if domain not in ('django', 'djangojs'):
        raise CommandError(
            "currently makemessages only supports domains 'django' and " \
            "'djangojs'")
    ...
    # this results in a utf-8 re-encoding of the original utf-8 that needs to
    # be undone to restore the original utf-8. So we check the xgettext
    # version here once and set a flag to remember if a utf-8 decoding needs
    # to be done on xgettext's output for Python files. We default to assuming
    # this isn't necessary if we run into any trouble determining the version.
    ...
    for locale in languages:
        ...
        for dirpath, file in all_files:
            ...
            if domain == 'djangojs' and file_ext == '.js':
                ...
                cmd = 'xgettext -d %s -L Perl --keyword=gettext_noop ' \
                      ...
                      '--copyright-holder="Zuza Software Foundation ' \
                      '(Translate.org.za)" -o - "%s"' % (
                ...
                if file_ext in extensions:
                    src = open(os.path.join(dirpath, file), "rU").read()
                    thefile = '%s.py' % file
                    open(os.path.join(dirpath, thefile), "w").write(
                        templatize(src))
                if verbosity > 1:
                ...
                cmd = 'xgettext -d %s -L Python --keyword=gettext_noop ' \
                      ...
                      '--keyword=ungettext_lazy:1,2 --from-code UTF-8 ' \
                      '--add-comments=l10n --copyright-holder=' \
        ...
        if os.path.exists(potfile):
            (stdin, stdout, stderr) = os.popen3(
                'msguniq --to-code=utf-8 "%s"' % potfile, 't')
            msgs = stdout.read()
            errors = stderr.read()
            if errors:
                raise CommandError(
            ...
            if os.path.exists(pofile):
                (stdin, stdout, stderr) = os.popen3(
                    'msgmerge -q "%s" "%s"' % (pofile, potfile), 't')
                msgs = stdout.read()
                errors = stderr.read()
                if errors:
                    raise CommandError(
...
class Command(BaseCommand):
    option_list = BaseCommand.option_list + (
        make_option(
            '--locale', '-l', default=None, dest='locale',
        ...
        make_option(
            '--verbose', action='store', dest='verbosity',
            default='1', type='choice', choices=['0', '1', '2'],
            help='Verbosity level; 0=minimal output, 1=normal output, ' \
            '2=all output'),
        make_option(
            '--all', '-a', action='store_true', dest='all', default=False,
            help='Reexamines all source code and templates for new ' \
    ...
    help = "Runs over the entire source tree of the current directory " \
           "and pulls out all strings marked for translation. It creates " \
           "(or updates) a message file in the conf/locale (in the django " \
           "tree) or locale (for project and application) directory."
    ...
    def handle(self, *args, **options):
        ...
        if '.js' in extensions:
            raise CommandError(
                "JavaScript files should be examined by using the special " \
                "'djangojs' domain only.")
