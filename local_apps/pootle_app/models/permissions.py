...
def get_permission_contenttype():
    content_type = ContentType.objects.filter(
        name='pootle', app_label='pootle_app', model="directory")[0]
    return content_type
    ...
    if pootle_path not in permissions_cache:
        try:
            permissionset = PermissionSet.objects.filter(
                ...
                profile__user__username=username).order_by(
                '-directory__pootle_path')[0]
        except IndexError:
            permissionset = None

        if len(path_parts) > 1 and path_parts[0] != 'projects' and \
               (permissionset is None or len(filter(
...
def get_matching_permissions(profile, directory):
    if profile.user.is_authenticated():
        permissions = get_permissions_by_username(
            profile.user.username, directory)
...
def check_profile_permission(profile, permission_codename, directory):
    """
    it checks if current user has the permission the perform
    C{permission_codename}
    """
...
class PermissionSet(models.Model):
    ...
    profile              = models.ForeignKey(
        'pootle_profile.PootleProfile', db_index=True)
    directory            = models.ForeignKey(
        'pootle_app.Directory', db_index=True, related_name='permission_sets')
    ...
    negative_permissions = models.ManyToManyField(
        Permission, editable=False, related_name='permission_sets_negative')
    ...
    def to_dict(self):
        return dict(
            ...
            self.positive_permissions.iterator())
